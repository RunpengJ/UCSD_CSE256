...
class Utilities:
    ...
    def sanity_check(self, sentence, block_size):
        ...
        print("Shape of each attention map:", attn_maps[0].shape)

        head_to_view = 0

        # Visualize and save the attention maps
        for j, attn_map in enumerate(attn_maps):
            att_map = attn_map.squeeze(0)[head_to_view].detach().cpu().numpy()  # Remove batch dimension and convert to NumPy array

            # Check if the attention probabilities sum to 1 over rows
            total_prob_over_rows = torch.sum(attn_map[0][head_to_view], dim=1)
            ...
            # Save the plot
            plt.savefig(os.path.join("../results", f"attention_map_{j + 1}.png"))
